...
- **Windows**：默认热键 `RightCtrl`，支持「按住/双击」同样行为，并提供可配置热键。
...
- Windows：WH_KEYBOARD_LL 监听 `RightCtrl` 并可配置
